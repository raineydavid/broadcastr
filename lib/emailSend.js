...
exports.send = function(user,client,recip,mergeFields,subj,body,callback){
  var errorCount = 0,
      sendCount = 0,
      tokens = user.tokens.filter(function(d){
        return d.type==="gmail"
      })[0];
    ...
    function(tokenCb){
      async.eachLimit(recip,1,function(d,callback){
        ...
        async.series([
          ...
          function(seriesCb){
            dbClient.query("INSERT INTO echo.activity_logs (timestamp,datecode,sender_id,sender_email,body,recipient_email,action,client_id) VALUES (current_timestamp,$1,$2,$3,$4,$5,'send',$6)",[uncodedDate,user.id,user.email,uniqueBody,d.email,client.client_id],function(queryErr){
